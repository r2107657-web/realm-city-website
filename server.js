...
async function initDB() {
  ...
  /* ---------- جدول الباقات (يتحكم فيه الأونر/الأدمن) ---------- */
  await pool.query(`
  ...
  /* ---------- تعبئة الباقات الافتراضية أول مرة فقط ---------- */
  ...
  /* ---------- إنشاء حساب الأونر الأول لو ما فيه أي حساب إدارة ---------- */
  ...
  if (adminCount[0].c === 0) {
    if (process.env.OWNER_EMAIL && process.env.OWNER_PASSWORD) {
      ...
      console.log('   (يفضل تسجل دخول وتغيّر كلمة المرور من لوحة الإدارة بعد أول دخول)');
    } else {
      console.warn('⚠️  ما فيه أي حساب إدارة بعد. عبّي OWNER_EMAIL و OWNER_PASSWORD بملف .env وأعد تشغيل السيرفر عشان يتنشئ حساب الأونر تلقائيًا.');
    }
  }
...
/* =====================================================
   Middlewares — المصادقة والصلاحيات
   ===================================================== */
...
/* =====================================================
   3) endpoint يستخدمه سيرفر الـFiveM
   ===================================================== */
app.get('/api/check-discord/:discordId', async (req, res) => {
  try {
    const [rows] = await pool.query(
...
/* =====================================================
   5) الباقات — عرض عام (يستخدمه الموقع الرئيسي)
   ===================================================== */
...
/* =====================================================
   6) تسجيل الدخول / الخروج للوحة الإدارة
   ===================================================== */
...
/* =====================================================
   7) إدارة حسابات الإدارة (أونر/أدمن) — الأونر فقط
   ===================================================== */
...
/* =====================================================
   9) إدارة طلبات الويتلست — أونر وأدمن
   ===================================================== */
...
/* =====================================================
   10) سجل العمليات (اللوق) — أونر وأدمن
   ===================================================== */
